...
//	$myuser->sec_session_start();
	...
	$query = "SELECT industry FROM tusers WHERE id = :idclient LIMIT 1";
	$stmt = $dbo->prepare($query);
	$stmt->bindParam(':idclient', $idclient, PDO::PARAM_INT);
	$stmt->execute();

	$industry = $stmt->fetchColumn();
	...
	$query = "SELECT d.id, d.title as Title, d.filename AS FileName, d.hostpath, date_format(d.created_at, '%d/%m/%Y %h:%i %p') AS Uploaded, d.deleted, 
	d.comments AS Comments, d.signature AS Signature, if (u.isclient=0, 'Admin', u.name) AS UserName, CONCAT(cl.name, ' (', cl.prefix, cl.id, ')') AS ClientName, cy.name AS CycleName
FROM tdocs AS d
LEFT OUTER JOIN tusers AS u ON d.iduser = u.id		
LEFT OUTER JOIN tusers AS cl ON d.idclient = cl.id		
LEFT OUTER JOIN tapplications AS app ON d.idapp = app.id
LEFT OUTER JOIN tcycles AS cy ON app.idcycle = cy.id
LEFT OUTER JOIN tdocs AS c ON (d.id = c.idparent AND c.deleted = 0)		

WHERE 1=1
$criteria
AND d.filename NOT LIKE '%.xls'
AND d.filename NOT LIKE '%.xlsx'
ORDER BY cl.name ASC,  cy.id DESC, d.created_at DESC
...
LIMIT ".$pageNumber." ,".$pageSize;
	...
	foreach ($data as $key=>$val) {
		...
		if ($data[$i]["Title"] == 'Client Questionnaire' && $industry == "Slaughter Houses") {
			$data[$i]['FileNameDE'] = '<a href="/files/docs/Customer_questionnaire_for_Slaughtering_DE.pdf" target="_blank"> <img src="/img/'.$icon.'" style="max-width:35px;" />Customer_questionnaire_for_Slaughtering_DE.pdf</a>';	
		}

		if ($data[$i]["Title"] == 'Checklist' && $industry == "Slaughter Houses") {
			$data[$i]['FileNameDE'] = '<a href="/files/docs/checklist_slaughtering_plants_DE.pdf" target="_blank"> <img src="/img/'.$icon.'" style="max-width:35px;" />checklist_slaughtering_plants_DE.pdf</a>';	
		}
