...
try {
	...
	$sql = 'select count(DISTINCT(i.itemcode))  as count from tclientactions i '
	 .' inner join tusers c on c.id=i.idclient 
     inner join tingredients g on i.itemid=g.id 
     '
	 .$filter.' AND g.deleted=0';
	$rows = $dbo->prepare($sql);
	$rows->execute();
    $totalRows = $rows->fetch(PDO::FETCH_ASSOC);

    $firstRowIndex = $curPage * $rowsPerPage - $rowsPerPage;
    $sql = 'select i.id, itemid, itemcode, itemtype, c.id as idclient, g.name as itemname, c.name as client, action, i.status, i.created_at 
    FROM tclientactions i '
					 .' inner join tusers c on c.id=i.idclient 
                     inner join tingredients g on i.itemid=g.id 
                     
                     '
           .$filter.' AND g.deleted=0 GROUP BY itemcode ORDER BY '.strtolower(str_replace(' ', '', $sortingField)).' '.$sortingOrder.' LIMIT '.$firstRowIndex.', '.$rowsPerPage;
    ...
    while($row = $res->fetch(PDO::FETCH_ASSOC)) {
        ...
        $response->rows[$i]['cell']=array($row['id'], $row['itemid'],$row['itemtype'],$row['idclient'], $row['created_at'], $row['client'],
            $row['itemcode'],  $row['itemname'], $row['action'], $row['status']);
        $i++;
    }
