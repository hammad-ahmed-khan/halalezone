...
/*
    ...
    */
    ...
    $sql = 'SELECT t.id, t.user_id, t.username, t.email, t.status, t.issue_type, t.issue_description, t.current_url, t.attachments, date_format(t.created_at, \'%d/%m/%Y %h:%i %p\') AS date_created, date_format(t.updated_at, \'%d/%m/%Y %h:%i %p\') AS last_updated, last_updated_by_name, last_updated_by_email
    from ttickets t ' . $filter;
    ...
    while($row = $res->fetch(PDO::FETCH_ASSOC)) {
        ...
        FROM treplies r WHERE r.ticket_id = :ticket_id ORDER BY r.created_at ASC';

        $replies_res = $dbo->prepare($replies_sql);
        $replies_res->bindParam(':ticket_id', $row['id'], PDO::PARAM_INT);
