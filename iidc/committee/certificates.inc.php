...
<script type="text/javascript">
    ...
    function checkMassCrtNr() {
        if (jQuery("#certificateItems").find(".massCrtNr:checked").length > 0) {
            jQuery("#callForMeetingForMass").css("display", "inline-block")
        } else {
            jQuery("#callForMeetingForMass").css("display", "none")
        }
    }

    function loadMassInput() {
        jQuery("#certificateItems").find(".aunr").each(function() {
            id = jQuery(this).parents("tr").prop("id").replace("tr_", "");
            jQuery(this).html('<input type="checkbox" class="massCrtNr" name="crtNrs[]" value="' + id + '" />')
        })

        jQuery("#certificateItems").find(".massCrtNr").each(function() {
            ...
            <th colspan=7 style="padding:0px 10px;">
                ...
                <span style="float:right">
                    <form id="searchAnnualCertificates" onsubmit="loadCertificates('yes');return false">
                        ...
                        <input type="hidden" id="referer" name="ref" value="committee" />
...
<div id="callForMeetingForMass" style="display: none;">
    <a href="" onclick="this.href = getMassInput()" target="iframe" data-width="1080" data-height="580" title="Call for a decision committee meeting"><i class="fa fa-user-clock"></i> Call for committee meeting</a>
</div>
<script>
    function getMassInput() {
        crtNrs = [];
        jQuery("#certificateItems").find("input[name='crtNrs[]']:checked").each(function() {
