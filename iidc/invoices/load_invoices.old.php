...
$sql = "SELECT nr,invoices.sbsid,invoices.clid,bclid,invoice_nr,invoice_type,service_type,invoice_items,subtotal,vat,total,paid_on,inserted_on,reminded_on,resent_on,credit_invnr,invoices.status,companies.company_name,companies.active,companies.offid FROM `invoices`
...
foreach ($invoices as $row) {
	if ($row['status'] == 'active' or $row['status'] == 'credited') {
		$nr++;
		$invoices_count++;
		if ($row['invoice_type'] == "credit_note" and $row['total'] > 0) {
	...
	?>
		<tr id="inv_<?php echo $row['nr']; ?>">
			...
			<td class="load_popup" data-url="../../admin/load_company.php?clid=<?php echo $row['clid']; ?>" title="<?php echo $row['company_name']; ?>">
			...
			<td>
				...
				if (file_exists($prog_path . $invFile)) { ?>
					...
					<?php } elseif ($row['invoice_nr'] == 'draft') { ?>DRAFT INVOICE<?php } else { ?>
			...
			<td>
				<?php echo $service_types[$row['invoice_type']]; ?>
				<?php if ($row['invoice_type'] == 'credit_note' && trim($row['credit_invnr']) != '') {
					$invFile = "/client_data/invoices/$row[credit_invnr].pdf";
					if (file_exists($prog_path . $invFile)) { ?>
						for: <a href="<?php echo $prog_www; ?>/client_data/invoices/<?php echo $row['credit_invnr']; ?>.pdf" target="_blank"><?php echo $row['credit_invnr']; ?></a>
					<?php } else { ?>
						for: <a href="pdf/pdf_create_invoice.php?nr=<?php echo $row['nr']; ?>" target="_blank"><?php echo $row['credit_invnr']; ?></a><?php };
																																				}; ?>
			</td>
				...
				<?php if ($_POST['show'] != 'credit') { ?>
					<td style="white-space:nowrap" class="noCursor<?php echo ($row['status'] != 'credited') ? ' status' : '' ?>">
						<?php if ($row['status'] == 'credited') { ?>
							<div style="color:#bbb">Credited</div>
							<i class="fas fa-file-invoice-dollar" title="Credited"></i>
							<?php
							$invFile = "/client_data/invoices/{$row['credit_invnr']}.pdf";
							if (file_exists($prog_path . $invFile)) { ?>
						...
						<?php } elseif (trim($row['invoice_type']) == 'credit_note') {
								...
								if ($invoice_due_date < 0) {
									echo '<div style="color:#900"><i class="far fa-calendar-minus" style="color:#900" title="Over Due"></i>' . $invoice_due_date . ' Days</div>';
									if (count($row['reminded_on']) > 0) {
										echo '<div><i class="far fa-bell" title="First reminder"></i>' . $row['reminded_on'][0] . '</div>';
									}
									if (count($row['reminded_on']) > 1) {
										$reminder_count = count($row['reminded_on']) + 1;
										echo '<div><i class="far fa-bell" style="color:orange" title="Reminder (' . $reminder_count . ')"></i>' . $row['reminded_on'][1] . '</div>';
									}
					...
					<?php if (isset($invoice_actions)) { ?>
						<td align="left" style="padding-left:5px;white-space:nowrap" class="action" data-id="<?php echo $row['nr']; ?>">
							<?php if ($row['invoice_nr'] == 'draft') { ?>
								<a href="index.php?inc=create_invoice&act=draft&nr=<?php echo $row['nr']; ?>&goback=invoices&show=all" title="Create invoice"><i class="fas fa-file-invoice"></i></a>
								<i class="fa fa-trash-alt" title="Delete draft invoice" data-id="<?php echo $row['nr']; ?>" data-act="deleteDraft" data-save="invoice_save.php"></i>
							<?php } else { ?>
								<a href="index.php?inc=resend_invoice&nr=<?php echo $row['nr']; ?>&act=rem" target="iframe" data-width="890" data-height="580" id="resend_<?php echo $row['nr']; ?>" title="Resend the INVOICE: (<?php echo $row['invoice_nr']; ?>)"><i class="far fa-paper-plane" <?php echo trim($row['resent_on']) != '' ? 'style="color:#2ad02a"' : "" ?>></i></a>
								<?php
...
?>
<tr>
	<th colspan="4" style="text-align:right;color:green;font-weight:normal">Paid invoices (<?php echo $paid_count; ?>):</th>
	<th style="color:green;font-weight:normal">&euro;<?php echo number_format($subtotal_paid, 2, ',', '.'); ?></th>
	<th style="color:green;font-weight:normal">&euro;<?php echo number_format($vat_paid, 2, ',', '.'); ?></th>
	<th style="color:green;font-weight:normal">&euro;<?php echo number_format($total_paid, 2, ',', '.'); ?></th>
	<th <?php echo ($colspan); ?>></th>
</tr>
<tr>
	<th colspan="4" style="text-align:right;color:red;font-weight:normal">Open invoices (<?php echo ($invoices_count - $paid_count); ?>):</th>
...
<tr>
	<th colspan="4" style="text-align:right">Totals (<?php echo $invoices_count; ?>):</th>
