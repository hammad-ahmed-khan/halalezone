...
//TODO: check this local and update it remote
...
$pdf_data['reference_number'] = $office['office_country'] . $office['certificate_prefix'] . str_pad($_REQUEST['clid'], 5, "0", STR_PAD_LEFT);

if (isset($_POST['invoice_number'])){
	$invoice_nr = $_POST['invoice_number'];
} else {
	$invoice_number_specs = json_decode($invoicing_office['invoice_number_specs'], true);
	$invoice_number = $invoicing_office['invoice_number']+1;
...
if ($_POST['act'] == "crt" or $_POST['act'] == 'test' or $_POST['act'] == 'save_draft' or $_POST['act'] == 'update_draft' or $_POST['act'] == 'save_scheduled' or $_POST['act'] == 'update_scheduled') {
	...
	if ($_POST['act'] != "test") {
		if (!isset($_POST['update'])) {
			...
			if ($_POST['act'] == "crt")
				$amdb->query("UPDATE hqc_invoice_nrs set invoice_number='$invoice_number' WHERE sbsid='$_POST[sbsid]'");
			else
				$invoice_nr = str_replace(array('save_', 'update_'), '', $_POST['act']);
		...
		if (
			$_POST['act'] == 'save_draft' or $_POST['act'] == 'update_draft' or
			$_POST['act'] == 'save_scheduled' or $_POST['act'] == 'update_scheduled'
		)
			exit();
	}
	...
	if (isset($_POST['mail_post']) and $_POST['mail_post'] == 'mail') {
		...
		if ($row = $amdb->get_row("SELECT * FROM invoice_templates WHERE template_name='$template_name'")) {
			...
			if (trim($client_name) != '')
				...
				$body['client_name'] = '';
			if (trim($_POST['email_message']['body']) != '') {
				$style = '';
			...
			if ($_POST['act'] == "test" and trim($_POST['test_email']) != '')
