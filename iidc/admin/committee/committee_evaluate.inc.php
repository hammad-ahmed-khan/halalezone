<?php
// Here is a reference snippet of code from admin\committee\committee.inc.php:
if ($member = $amdb->get_row("SELECT * FROM hqc_committee_members WHERE comemid='$_GET[comemid]' AND status='active'")) {
    $evaluation = trim($member['evaluation']) != '' ? unserialize($member['evaluation']) : [];    // Get the evaluation data
...
?>
    ...
    <h2 class="content_title" style="text-align:center;">Committee member Evaluation Form</h2>

    <form id="evaluationForm" target="" action="committee_evaluate_save.php" method="post" onsubmit="post_this_form(this)">
        <input type="hidden" name="comemid" value="<?php echo $_GET['comemid']; ?>" />
        <input type="hidden" name="act" value="update_evaluation" />
        <h4 class="form-group">
            <b>Member Name:</b> <?php echo $member['member_name']; ?>
        </h4>

        <fieldset class="pros">
            <legend>Pros (Advantages of having <b><?php echo $member['member_name']; ?></b> on our team):</legend>
        ...
        <fieldset class="cons">
            <legend>Cons (Disadvantages of having <b><?php echo $member['member_name']; ?></b> on our team):</legend>
        ...
        <div style="text-align:center;">
            <button type="submit">Save Evaluation</button> <button type="button" onclick="location='?inc=committee'">Cancel Evaluation</button> <?php if (trim($member['evaluation']) != '') { ?><button type="button" onclick="removeEvaluation()">Remove Evaluation</button><?php }; ?>
        </div>
    </form>
    ...
    <script>
        ...
        async function removeEvaluation() {
            ...
            $.post("committee_evaluate_save.php", {
                act: "remove_evaluation",
                comemid: '<?php echo $_GET['comemid']; ?>'
            }, function(data) {
                if (data == 'success') {
                    location = '?inc=committee';
                } else {
                    alert_message(data);
                }
            });
