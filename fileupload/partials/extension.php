...
try {
	...
	if ($errors == "") {
		...
		$hostPath = $config['filesfolder']."/".$config['clientsfolder']."/".$client."/application/extension/";			
	...
	$user = $stmt->fetch(PDO::FETCH_ASSOC);
		...
		$body['subject'] = "Halal e-Zone - Certification Extension is Uploaded - ".$data["user"]["name"];
		//$body['header'] = "Client ".getClientInfo($prod['idclient'])." added a new product:";
		$body['message'] = "Dear ".$user["name"].",
<br /><br />
Your HQC Halal Certificate Extension has been uploaded. You can download it at the Halal eZone portal. 
<br /><br />
Best regards,<br />
Your HQC supporting Team";
sendEmailWithAttach($body);

insertActivityLog($idclient, $idapp, $myuser->userdata['id'], $myuser->userdata['name'], 'Halal Certificate extension uploaded');
