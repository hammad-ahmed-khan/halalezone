...
try {
	...
	$Title = 'Signed Audit Review Report';
	...
	if ($errors == "") { 
		...
		$hostPath = $config['filesfolder']."/".$config['clientsfolder']."/".$client."/application/review/";			
	...
	$user = $stmt->fetch(PDO::FETCH_ASSOC);
		...
		$ownerEmailAddress = "halal.ezone@gmail.com";
					...
					$body['message'] .= '<p>Please find attached to this email our '.$Title.'</p>
					<p>Regards,<br/>Your HQC Team</p>
					';					
					//sendEmailWithAttach($body);
					insertActivityLog($idclient, $idapp, $myuser->userdata['id'], $myuser->userdata['name'], 'Signed audit review report uploaded');
	}
