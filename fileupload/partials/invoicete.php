...
try {
	...
	$Title = 'Invoice for travel expenses';
	...
	if ($errors == "") { 
		...
		$hostPath = $config['filesfolder']."/".$config['clientsfolder']."/".$client."/application/invoicete/";			
	...
	$user = $stmt->fetch(PDO::FETCH_ASSOC);
		...
		$ownerEmailAddress = "halal.ezone@gmail.com";
					...
					$body['header'] = "";
					$body['message'] = "<p>Dear ".$user["name"]."</p>";
					$body['message'] .= "<p><strong>".$myuser->userdata['name']."</strong> has uploaded ".$Title.". Please find attached to this email.</p>
					<p>Regards,<br/>Halal Digital</p>
					";					
					sendEmailWithAttach($body);


					insertActivityLog($idclient, $idapp, $myuser->userdata['id'], $myuser->userdata['name'], 'Invoice for travel expenses uploaded');
	}
