(function ($) {
  var default_options = {
    ...
    background: false,
    templateFile:
      '<div id="fileup-[INPUT_ID]-[FILE_NUM]" class="fileup-file [TYPE]" style="width:100%; max-width:100%; margin-top:15px;">' +
  ...
  function dropFiles(event) {
    ...
    if (files.length) {
      ...
      for (var i = 0; i < files.length; i++) {
        var file = files[i];
        //alert(Object.keys(files).length+" <=> "+options.filesLimit);
      ...
      if (!multiple) {
        if (events.callEvent(this, "remove", ["*", "1", this.files[0]])) {
          ...
          if (options.background) {
            $("#" + options.queueID).append("<h3>Uploading...</h3>");
          }
          this.fileup.files = {};
        } else {
          $(this).val("");
