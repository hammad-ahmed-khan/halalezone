...
<div>
  <h1>Completed tasks waiting for confirmation</h1>
  ...
  <table border="1" cellspacing="0">
    ...
    <tbody>
      <?php
        foreach($client['tasks'] as $item) {
          echo "<tr><td>".date('j M Y', strtotime($item['created_at']))."</td>".
          '<td><a href="'.$GLOBALS['httpprefix'].'ingredients?id='. $item['idingredient'] .'&idclient='.$item['idclient'].
          '" target="_blank">RMC_'.$item['idingredient'].'</a></td>'.
          '<td><a href="'.$GLOBALS['httpprefix'].'ingredients?id='. $item['idingredient'] .'&idclient='.$item['idclient'].
